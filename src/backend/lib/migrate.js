...
export default async (version, addNewResources) => {
	...
	if( Bun.semver.satisfies(version, '<0.4.0') ){
		await settings.set('antidpi.args', (
			await settings.get('startup.args')
		));
		
		await settings.delete('startup.args');

		await settings.set('dns.active', true);
		await settings.set('dns.nameserver', '8.8.8.8');
	...
	if( Bun.semver.satisfies(version, '<0.4.4') ){
		if( process.platform === 'win32' ){
			await settings.set('antidpi.args', '--wf-tcp-out=80,443,1024-65535 --wf-udp-out=443,1024-65535');
		}

		if( addNewResources ){
			await settings.set('dns.doh-url', 'https://xbox-dns.ru/dns-query');
