...
-- arg : start_with=<hex> - replace beginning of packet
...
function mangled(ctx, desync)
    local function parse_segment(segment, payload)
        ...
        return unpack(resolve_range(payload, desync.l7payload, from .. ',' .. to))
    end
            ...
            for i = 1, qty do
                ...
                if desync.arg.rnd then
                    for segment in string.gmatch(desync.arg.rnd, "([^,]+)") do
                        randomize_bytes(fakebytes, parse_segment(segment, payload))
                    end
                end

                if desync.arg.start_with then
                    local prefix = parse_hex(string.sub(desync.arg.start_with, 3))
